...
export class ShopingCartComponent implements OnInit{
  ...
  sendEmail() {
    this.Cart.itemsProduct.forEach(item => {
      if(item.itemProduct.productQuantity <= 2){
        const emailData = {
          to: this.userEmail,
          subject: 'Jiso Store Communtity',
          body: 'Hello Mr '+ this.user.userName + 
          '. The product '+ item.itemProduct.productTitle +' in you cart is almost sold out.' 
