...
export class AdminComponent {
    ...
    applyDiscountStatus : boolean = false;
  
    user : any;

    offer0 : Offer = new Offer(UUID.UUID(),'Discount', 'Apply discount on all product', 0, false, '');
    ...
    applyDiscount(status : boolean){
      if(status == true){
        let percentage = Number(prompt('Enter the percentage of th discount'));
        this.productService.applyDiscount(status,percentage).subscribe({
          next : () => {
            this.applyDiscountStatus = !this.applyDiscountStatus;
        ...
        //this.applyDiscountStatus = !status;
      }
      else{
        this.productService.applyDiscount(false,0).subscribe({
          next : () => {
            this.showNotification('Discount removed successfully');
          },
          error(err) {
            console.error(err);
          }
